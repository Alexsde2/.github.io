...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Para el amor de mi vida:\n\n Mi niña hermosa quiero que sepas que cada vez que te digo que te amo no es solo por decir, yo realmente te elijo a ti como mi compañera de vida eres lo mejor que me ha pasado. Nunca crei amar o sentir tanto por alguien como lo que siento yo por ti.Confio en que un dia tu seras mi esposa y que me permitas estar siempre a tu lado y si quiero quesas la mdre de nuestros bbys `;  } else {
    text = decodeURIComponent(text).replace(/\\n/g, '\n');
  }
  const container = document.getElementById('dedication-text');
  if (!container) return; // Si no existe el contenedor, salir sin lanzar errores
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) :"Con amor, tu niño hermoso";
...
function showCountdown() {
  ...
  // Usar ISO (YYYY-MM-DD) por defecto; aceptar parámetros YYYY-MM-DD
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-06-23T00:00:00');
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-06-23T00:00:00');
  // Si las fechas no son válidas, usar la fecha actual para evitar NaN
